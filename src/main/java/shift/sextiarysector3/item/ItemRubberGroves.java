...
public class ItemRubberGroves extends ItemSSBase {
    ...
    public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos_o, EnumHand hand, EnumFacing facing_o, float hitX_o, float hitY_o, float hitZ_o) {
        ...
        if (raytraceresult != null) {
            ...
            hitX = (float) (raytraceresult.hitVec.x - (double) pos.getX());
            hitY = (float) (raytraceresult.hitVec.y - (double) pos.getY());
            hitZ = (float) (raytraceresult.hitVec.z - (double) pos.getZ());
        }
